...
drop trigger if exists system_config_delete_trigger; 
    ...
    VALUES (NULL, 'delete', NOW(), old.variable, old_paramsVal);
          ...
          use cognitivesvcx;
		  drop table if exists person_face_feature_logs;
	      create table person_face_feature_logs
			(  id  int auto_increment
				primary key,
				operation      varchar(30) not null,
				operation_time datetime    not null,
				...
				charset = utf8;
drop trigger if exists person_face_feature_insert_trigger;
	 create  trigger person_face_feature_insert_trigger
		after insert
		on cognitivesvcx.person_face_feature
		for each row
			BEGIN
				DECLARE operate_paramsVal TEXT DEFAULT '';

				SET operate_paramsVal = CONCAT(' The insertion reads: ' ,'  ,id:', NEW.id  ,'  ,avatar_image_url:', NEW.avatar_image_url  ,'  ,image_feature:', NEW.image_feature  ,'  ,model_version:', NEW.model_version  ,'  ,person_uuid:', NEW.person_uuid  ,'  ,person_cn_name:', NEW.person_cn_name  ,'  ,person_en_name:', NEW.person_en_name  ,'  ,sts:', NEW.sts  ,'  ,create_user:', NEW.create_user  ,'  ,create_ts:',  IF(NEW.create_ts IS NOT NULL,DATE_FORMAT(NEW.create_ts, '%Y-%m-%d %H:%i:%s'),'null')  ,'  ,last_mod_user:', NEW.last_mod_user  ,'  ,last_mod_ts:',  IF(NEW.last_mod_ts IS NOT NULL,DATE_FORMAT(NEW.last_mod_ts, '%Y-%m-%d %H:%i:%s'),'null')  ,'  ,tag:', NEW.tag  ,'  ,privilege:', NEW.privilege   ); -- 列出所有字段并获取NEW中的对应值

				INSERT INTO person_face_feature_logs (id, operation, operation_time, operation_id, operate_params)
				VALUES (NULL, 'insert', NOW(), NEW.id, operate_paramsVal);
			END;
drop trigger if exists person_face_feature_update_trigger;
	 create trigger person_face_feature_update_trigger
				after update
				on cognitivesvcx.person_face_feature
				for each row
			begin
				 DECLARE old_params TEXT DEFAULT '';
				  DECLARE new_params TEXT DEFAULT '';
				SET old_params = CONCAT(' Before the update it read: ' ,'  ,id:', NEW.id  ,'  ,avatar_image_url:', NEW.avatar_image_url  ,'  ,image_feature:', NEW.image_feature  ,'  ,model_version:', NEW.model_version  ,'  ,person_uuid:', NEW.person_uuid  ,'  ,person_cn_name:', NEW.person_cn_name  ,'  ,person_en_name:', NEW.person_en_name  ,'  ,sts:', NEW.sts  ,'  ,create_user:', NEW.create_user  ,'  ,create_ts:',  IF(NEW.create_ts IS NOT NULL,DATE_FORMAT(NEW.create_ts, '%Y-%m-%d %H:%i:%s'),'null')  ,'  ,last_mod_user:', NEW.last_mod_user  ,'  ,last_mod_ts:',  IF(NEW.last_mod_ts IS NOT NULL,DATE_FORMAT(NEW.last_mod_ts, '%Y-%m-%d %H:%i:%s'),'null')  ,'  ,tag:', NEW.tag  ,'  ,privilege:', NEW.privilege  );
				SET new_params=CONCAT(' The updated content is:' ,'  ,id:', old.id  ,'  ,avatar_image_url:', old.avatar_image_url  ,'  ,image_feature:', old.image_feature  ,'  ,model_version:', old.model_version  ,'  ,person_uuid:', old.person_uuid  ,'  ,person_cn_name:', old.person_cn_name  ,'  ,person_en_name:', old.person_en_name  ,'  ,sts:', old.sts  ,'  ,create_user:', old.create_user  ,'  ,create_ts:', IF(NEW.create_ts IS NOT NULL,DATE_FORMAT(NEW.create_ts, '%Y-%m-%d %H:%i:%s'),'null')  ,'  ,last_mod_user:', old.last_mod_user  ,'  ,last_mod_ts:', IF(NEW.last_mod_ts IS NOT NULL,DATE_FORMAT(NEW.last_mod_ts, '%Y-%m-%d %H:%i:%s'),'null')  ,'  ,tag:', old.tag  ,'  ,privilege:', old.privilege   ); -- 列出所有字段并获取NEW中的对应值
			
					 INSERT INTO person_face_feature_logs (id, operation, operation_time, operation_id, operate_params)
				VALUES (NULL, 'update', NOW(), NEW.id, concat(old_params,new_params));
			end;
drop trigger if exists person_face_feature_delete_trigger; 
	create  trigger person_face_feature_delete_trigger
    after delete
    on cognitivesvcx.person_face_feature
    for each row
    begin
     DECLARE old_paramsVal TEXT DEFAULT '';
     SET old_paramsVal = CONCAT(' Deleted content: ' ,'  ,id:', old.id  ,'  ,avatar_image_url:', old.avatar_image_url  ,'  ,image_feature:', old.image_feature  ,'  ,model_version:', old.model_version  ,'  ,person_uuid:', old.person_uuid  ,'  ,person_cn_name:', old.person_cn_name  ,'  ,person_en_name:', old.person_en_name  ,'  ,sts:', old.sts  ,'  ,create_user:', old.create_user  ,'  ,create_ts:', IF(NEW.create_ts IS NOT NULL,DATE_FORMAT(NEW.create_ts, '%Y-%m-%d %H:%i:%s'),'null')  ,'  ,last_mod_user:', old.last_mod_user  ,'  ,last_mod_ts:', IF(NEW.last_mod_ts IS NOT NULL,DATE_FORMAT(NEW.last_mod_ts, '%Y-%m-%d %H:%i:%s'),'null')  ,'  ,tag:', old.tag  ,'  ,privilege:', old.privilege   );
	INSERT INTO person_face_feature_logs (id, operation, operation_time, operation_id, operate_params)
